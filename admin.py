...
def studentinfo():
    ...
    # cur.execute("SELECT  * FROM stud24")
    # data = cur.fetchfirst()
    # return render_template('studentdata.html', stud24=data)
    data=Stud24.query.all()
    print(data)
    for item in data:
        print(item)
    return render_template('studentdata.html', stud24 = Stud24.query.all() )
...
def studentinsert():
    ...
    try:
        if request.method == "POST":
            ...
            entry_student=Stud24(class_name=class_name,studying_subjects=studying_subjects,is_monitor=is_monitor)
            db.session.add(entry_student)
            db.session.commit()
            # cur.execute("INSERT INTO stud24(class_name,is_monitor,studying_subjects) VALUES (  %s, %s, %s)", (class_name, is_monitor, studying_subjects))
            # mysql.commit()                    
...
def studentdelete(id_data):
    # import pdb;pdb.set_trace()
    # cur.execute("DELETE FROM stud24 WHERE id=%s", (id_data))
    # mysql.commit()
    delete_student=Stud24.query.filter_by(student_id=id_data).first()
...
def studentupdate():
    try:
        if request.method == 'POST':
            ...
            studying_subjects=data.get('studying_subjects')
            update_student=Stud24.query.filter_by(student_id=id_data).first()
...
def teacherinfo():
    # import pdb;pdb.set_trace()
    # cur.execute("SELECT  * FROM teacher24")
    # data = cur.fetchfirst()
    # return render_template('teacherdata.html', teacher24=data)
    return render_template('teacherdata.html', teacher24 = Teacher24.query.all() )
    ...
    try:
        if request.method == "POST":
            ...
            if data.get('is_hod')=="0":
                ...
                is_hod=1
            # cur.execute("INSERT INTO teacher24(department_name,is_hod,teaching_subject) VALUES (  %s, %s, %s)", (department_name, is_hod, teaching_subject))
            # mysql.commit()                    
            entry_teacher=Teacher24(department_name=department_name,is_hod=is_hod,teaching_subject=teaching_subject)
...
def teacherdelete(id_data):
    # import pdb;pdb.set_trace()
    # cur.execute("DELETE FROM teacher24 WHERE id=%s", (id_data))
    # mysql.commit()
    delete_teacher=Teacher24.query.filter_by(teacher_id=id_data).first()
...
def teacherupdate():
    try:
        if request.method == 'POST':
            ...
            # mysql.commit()
            update_teacher=Teacher24.query.filter_by(teacher_id=id_data).first()
...
def userinfo():
        # import pdb;pdb.set_trace()
        # cur.execute("SELECT  * FROM user24")
        # data = cur.fetchfirst()
        # return render_template('userdata.html', user24=data)

        
        return render_template('userdata.html', user24 = User24.query.all() )
...
def userinsert():
    try:
        if request.method == "POST":
            ...
            registered_user = User24.query.filter_by(mobileno=mobileno1).first()
            if registered_user is not None:
                return jsonify({'result':'Mobile number already exist.'}) 
            else:
                import pdb; pdb.set_trace()
                entry_user=User24(fname=fname,lname=lname,email=email,mobileno=mobileno1,about=about)
                db.session.add(entry_user)
                db.session.commit()
                return jsonify({'result':'User Data added successfully......' })
    except:
            return jsonify({'result':'Something went wrong..........' }) 
            # cur.execute("SELECT mobileno FROM user24 WHERE mobileno=%s", (mobileno1))
            # row=cur.fetchone()
            # if row is not None:            
            #     return jsonify({'result':'Mobile number already exist.'})
            # else:
            #      cur.execute("INSERT INTO user24(fname,lname,email,mobileno,about) VALUES (  %s, %s, %s, %s, %s)", (fname, lname, email, mobileno1, about))
            #      mysql.commit()      
...
def userdelete(id_data):
    # cur.execute("DELETE FROM user24 WHERE id=%s", (id_data))
    # mysql.commit()
    delete_user=User24.query.filter_by(user_id=id_data).first()
...
def userupdate():
    ...
    try:
        if request.method == 'POST':
            ...
            # mysql.commit()
            update_user=User24.query.filter_by(user_id=id_data).first()
